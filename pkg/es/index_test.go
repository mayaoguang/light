...
func TestNewClient(t *testing.T) {
	...
	// add data
	light := Light{UserId: "12345678", Remark: "备注1", Amount: decimal.NewFromInt(10), CreateTime: time.Now().Unix()}
	if _, err = NewClient().Index().Index(indexName).Type(indexType).BodyJson(light).Do(context.Background()); err != nil {
		t.Error(err)
	}
	time.Sleep(1 * time.Second)
	// query
	query := elastic.NewTermQuery("userId", "12345678")
	t.Log(query.Source())
